...
public class WorkersDAO implements IDAO{
    ...
    public DTO getObj(String[] keys) {
        ...
        if(workerIM.getCachedObj(keys[0]) != null)
            ...
            List<String[]> WorkerFromDb = dbc.select(TableName, new String[]{params[0]},keys);
            if(WorkerFromDb.isEmpty())
                throw new IllegalArgumentException("There is no worker with id:"+keys[0]);
            else
            {
                wDTO = new WorkerDTO(WorkerFromDb.get(0)[0],WorkerFromDb.get(0)[1],WorkerFromDb.get(0)[2],WorkerFromDb.get(0)[3],WorkerFromDb.get(0)[4],WorkerFromDb.get(0)[5],WorkerFromDb.get(0)[6],WorkerFromDb.get(0)[7]);
